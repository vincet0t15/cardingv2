...
interface EmployeesByDeductionTypeProps {
    deduction_type: {
        id: number;
        name: string;
        code: string;
    };
    employees: EmployeeDeduction[];
    employees_paginated: PaginatedDataResponse<EmployeeDeduction>;
    summary: Summary;
...
export default function EmployeesByDeductionType({
    deduction_type,
    employees,
    employees_paginated,
    summary,
    ...
    employment_statuses,
    filters,
}: EmployeesByDeductionTypeProps) {
    const [searchValue, setSearchValue] = useState(filters.search || '');

    const handleFilterChange = (key: string, value: any) => {
        router.get(
            route('may-deductions.employees', deduction_type.id),
            {
                month: filters.month,
                year: filters.year,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${deduction_type.name} - Employees`} />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div>
                    <Button variant="outline" size="sm" className="inline-flex" onClick={() => router.get(route('may-deductions.index'), {}, { preserveState: true, preserveScroll: true })}>
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        Back to May Deductions
                    </Button>
                </div>

                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">{deduction_type.name}</h1>
                        <p className="text-muted-foreground mt-1">Employees with {deduction_type.name} deductions</p>
                    </div>
                    <Button onClick={() => window.open(route('may-deductions.employees.print', { deductionType: deduction_type.id, ...filters }), '_blank')}>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-muted-foreground text-xs">With {deduction_type.name} deductions</p>
                        </CardContent>
                    </Card>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            Employees with {deduction_type.name} Deductions
                        </CardTitle>
                        <CardDescription>List of employees who have {deduction_type.name} deductions</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {employees.length > 0 ? (
                            <>
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-right">Deduction Count</TableHead>
                                            ...
                                            <TableRow key={employee.id} className="hover:bg-muted/50">
                                                ...
                                                <TableCell>
                                                    <button
                                                        ...
                                                        onClick={() => router.get(route('manage.employees.index', employee.employee_id))}
                                                    >
                                                ...
                                                <TableCell className="text-right font-semibold">{employee.deduction_count}</TableCell>
