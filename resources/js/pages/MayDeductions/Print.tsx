...
interface MayDeductionsPrintProps {
...
export default function MayDeductionsPrint({ deductionTypes, summary, filters }: MayDeductionsPrintProps) {
    ...
    return (
        <div className="mx-auto min-h-screen bg-white p-4 font-sans text-[11px] leading-[1.3] text-black print:max-w-none print:p-0">
            <Head title="May Deductions Report - Print" />
                        ...
                        <tr>
                            <td>
                                ...
                                <table className="mb-6 w-full border-collapse border border-black">
                                    <tbody>
                                        <tr>
                                            <td className="border border-black p-2 font-bold">Total Deduction Types</td>
                                            <td className="border border-black p-2 text-right">{summary.total_deduction_types}</td>
                                            <td className="border border-black p-2 font-bold">Total Deductions</td>
                                            <td className="border border-black p-2 text-right font-medium">{summary.total_deductions}</td>
                                        </tr>
                                ...
                                {deductionTypes.length > 0 ? (
                                    <table className="w-full border-collapse border border-black">
                                        <thead>
                                            <tr className="bg-gray-100">
                                                ...
                                                <th className="border border-black px-2 py-1 text-left text-[10px] font-semibold">Deduction Type</th>
                                                <th className="border border-black px-2 py-1 text-left text-[10px] font-semibold">Code</th>
                                                <th className="w-28 border border-black px-2 py-1 text-right text-[10px] font-semibold">Deductions</th>
                                        ...
                                        <tbody>
                                            {deductionTypes.map((type, index) => (
                                                <tr key={type.id}>
                                                    ...
                                                    <td className="border border-black px-2 py-1 text-right text-[10px]">{type.deduction_count}</td>
                                            ...
                                            <tr className="bg-gray-100 font-bold">
                                                ...
                                                <td className="border border-black px-2 py-1 text-right text-[10px]">{summary.total_deductions}</td>
                                    ...
                                    <div className="py-12 text-center text-[11px]">No deductions data found for the selected filters</div>
                                )}
